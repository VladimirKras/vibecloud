...
test("observer failures do not escape a successful query or suppress other observers", () => {
  const statement = fakeStatement(undefined);
  const client = instrumentYdbClient((() => statement) as unknown as QueryClient, () => {
    throw new Error("export failed");
  });
  let observed = false;
  describeYdbQuery(client("SELECT 1"), "successful", {}, () => {
    observed = true;
  });
  assert.doesNotThrow(() => statement.emit("done", [[{ value: 1 }]]));
