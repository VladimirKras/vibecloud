...
function instrumentStatement(
  ...
  const complete = (success: boolean, returnedRows?: number) => {
    ...
    notifyObserver(() => observe(observation));
    notifyObserver(() => details?.observe?.(observation));
  };
  statement.on("done", (resultSets) => {
    notifyObserver(() => complete(true, resultSets.reduce((total, rows) => total + rows.length, 0)));
  });
  statement.on("error", () => {
    notifyObserver(() => complete(false));
  });
  return statement;
}
...
function notifyObserver(work: () => unknown): void {
  const report = () => logDatabase("WARN", "YDB query observer failed");
  try {
    const result = work();
