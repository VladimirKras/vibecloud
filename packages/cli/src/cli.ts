...
import { stopLocalProject } from "./local-runtime.ts";
import { devProject } from "./dev.ts";
import { migrateProjectDatabases, pushProject } from "./push.ts";
import { inspectProjectOrphans, createResourceScaffold, synchronizeProjectPackage } from "./scaffold.ts";
...
Templates and ready features:
  ...
  function --template ai-image
    Public synchronous Alice AI ART image generation endpoint.
...
Function templates:
  ...
  ai-image (aliases: image, illustrator)
    Public Alice AI ART endpoint. POST returns image bytes; no operation polling.
...
Notes:
  ...
  For AI without accounts, use api with @vibecloud/ai and explicit ai capabilities.
...
add
  ...
  .action(async (options) => {
    ...
    await withProjectMutation(dirname(dirname(configPath)), async () => {
...
database
  ...
  .option("-c, --config <path>", "configuration file", defaultConfig)
  .option("--retry-interrupted", "retry failed or stale migrations from the beginning; inspect partial effects and verify the previous run stopped first")
  .action(async (options) => {
    const loaded = await loadConfig(resolve(options.config));
    const migrated = await migrateProjectDatabases(loaded, { migrationRecovery: options.retryInterrupted ? "retry" : undefined });
...
add
  ...
  .option("--public", "allow anonymous reads of objects; writes remain authenticated")
  .option("-c, --config <path>", "configuration file", defaultConfig)
  .action(async (name, options) => {
    const configPath = resolve(options.config);
    await withProjectMutation(dirname(dirname(configPath)), async () => {
      printEdit(await addResource(configPath, "bucket", name, { public: options.public }));
...
add
  ...
  .option("--bucket <name>", "declared public bucket for ai-image; defaults to a generated public images bucket")
  .option("--runtime <runtime>", "function runtime")
  .option("--build-command <command>", "custom build command; must write to VIBECLOUD_FUNCTION_OUTPUT")
...
program
  .command("orphans")
  .description("report retained source and dependencies no longer required by declarations")
  .option("-c, --config <path>", "configuration file", defaultConfig)
  .action(async (options) => {
    const loaded = await loadConfig(resolve(options.config));
    const findings = await inspectProjectOrphans(loaded);
...
program
  .command("down")
  .description("stop this checkout's local containers; retain data unless --volumes is requested")
  .argument("[config]", "Terraform variable file", defaultConfig)
  .option("--volumes", "also delete this checkout's local database, media and dependency data")
  .option("--confirm <delete-local:name>", "required with --volumes")
  .action(async (config, options) => {
    const loaded = await loadConfig(resolve(config));
    await stopLocalProject(loaded, { volumes: options.volumes, confirmation: options.confirm });
    console.log(`local ${options.volumes ? "resources and data removed" : "preview stopped"}: ${loaded.config.name}`);
  });
...
program
  ...
  .argument("[config]", "Terraform variable file", defaultConfig)
  .option("--confirm <delete:name>", "exact full-project deletion confirmation (required unless --status)")
  .option("--delete-after <duration>", "managed folder deletion delay in HhMmSs (0s skips the grace period; default: YC's grace period)")
  .option("--status", "refresh managed folder deletion status without submitting a deletion")
  .option("--operation <id>", "recover an exact deletion operation with --status")
  .action(async (config, options) => {
    const configPath = resolve(config);
    const loaded = await loadConfig(configPath);
    const result = await deleteProject(loaded, {
      ...
      statusOnly: options.status,
      operationId: options.operation,
    });
    if (result.destroyed) console.log(`destroyed: ${loaded.config.name}`);
    if (result.deletionStatus !== undefined) {
      console.log(`YC folder deletion ${result.folderDeletionSubmitted ? "submitted" : "status"}: ${result.folderId}`);
      console.log(`status: ${result.deletionStatus}`);
      if (result.operationId) console.log(`operation: ${result.operationId}`);
      if (result.deleteAt) console.log(`delete after: ${result.deleteAt}`);
      if (result.deletionError) console.log(`operation error: ${result.deletionError}`);
      if (["submitting", "pending", "deleting"].includes(result.deletionStatus)) console.log("cleanup remains asynchronous; use vibecloud delete --status to check completion");
    }
  });
