import { updateTerraformMoves, type TerraformMove } from "./terraform-moves.ts";
import { commitProjectEdit } from "./project-edit.ts";
import { withProjectMutation } from "./project-lock.ts";
import { localResourceRenameEdit } from "./local-identities.ts";
...
interface BucketOptions { public?: boolean }
...
interface FileEdit { path: string, original: string | null, updated: string }

export async function addResource(
  configPath: string,
  kind: ResourceKind,
  name: string,
  options: FunctionOptions | AssetOptions | DatabaseOptions | BucketOptions = {},
) {
  ...
  return edit(configPath, `${kind} ${key}`, (config) => {
    const definitions = {
      ...
      bucket: () => ((options as BucketOptions).public === undefined ? {} : { public: (options as BucketOptions).public }),
      database: () => databaseDefinition(options as DatabaseOptions),
...
export async function renameResource(configPath: string, kind: ResourceKind, oldName: string, newName: string) {
  ...
  return edit(configPath, description, async (config, context) => {
    ...
    if (kind === "bucket") {
      context.fileEdits.push(await localResourceRenameEdit(dirname(dirname(configPath)), "buckets", oldKey, newKey));
      for (const [functionName, definition] of Object.entries(config.functions ?? {})) {
        if (definition.bucket !== oldKey) continue;
        definition.bucket = newKey;
        const module = definition.handler.slice(0, definition.handler.lastIndexOf("."));
        const sourceEdit = await resourceBindingEdit(join(dirname(dirname(configPath)), "src/functions", functionName, `${module}.ts`), oldKey, newKey, "BUCKET");
        if (sourceEdit) context.fileEdits.push(sourceEdit);
      }
    }
    if (kind === "database") {
      context.fileEdits.push(await localResourceRenameEdit(dirname(dirname(configPath)), "databases", oldKey, newKey));
      ...
      for (const [functionName, functionDefinition] of Object.entries(config.functions ?? {})) {
        ...
        const sourceEdit = await resourceBindingEdit(path, oldKey, newKey);
        if (sourceEdit) context.fileEdits.push(sourceEdit);
      }
    }
...
async function editUnlocked(
  ...
  let config = validateConfig(JSON.parse(originalConfig));
  ...
  if (action !== "unchanged") {
    ...
    const updatedMoves = updateTerraformMoves(originalMoves, context.terraformMoves, config);
    await commitProjectEdit(dirname(dirname(configPath)), {
      moves: context.fileMoves,
      writes: [
        ...context.fileEdits,
        { path: configPath, original: originalConfig, updated: `${JSON.stringify(config, null, 2)}\n` },
        ...(originalMoves === updatedMoves ? [] : [{ path: movesPath, original: originalMoves, updated: updatedMoves }]),
      ],
    });
  }
...
function resourceTerraformMoves(kind: ResourceKind, oldKey: string, newKey: string, definition: unknown) {
  ...
  return [
    ...(functionDefinition.template === "datastream-trigger" || functionDefinition.triggers?.length
      ? [
        move("yandex_function.functions", `stream-${oldKey}`, `stream-${newKey}`),
        move("yandex_function_iam_binding.invoker", `stream-${oldKey}`, `stream-${newKey}`),
      ]
      : []),
...
async function resourceBindingEdit(path: string, oldKey: string, newKey: string, suffix = "ENDPOINT"): Promise<FileEdit | undefined> {
  let original: string;
  try {
  ...
  const oldBinding = `${oldKey.toUpperCase().replaceAll("-", "_")}_${suffix}`;
  const newBinding = `${newKey.toUpperCase().replaceAll("-", "_")}_${suffix}`;
...
function functionDefinition(options: FunctionOptions) {
  ...
  return {
    ...
    ...(options.bucket === undefined ? {} : { bucket: validateResourceKey(options.bucket) }),
...
function functionDefinitionForConfig(config: VibecloudConfig, options: FunctionOptions) {
  if (options.template === "ai-image") {
    const bucket = options.bucket ?? "images";
    if (options.bucket === undefined) {
      config.buckets ??= {};
      config.buckets.images ??= { public: true };
    }
    return functionDefinition({ ...options, bucket });
  }
  ...
  const authDatabases = new Set(Object.values(config.functions ?? {})
    .filter((definition) => definition.features?.auth === "provider" && definition.database)
...
async function edit(...args: Parameters<typeof editUnlocked>): Promise<EditResult> {
  return withProjectMutation(dirname(dirname(args[0])), () => editUnlocked(...args));
}
