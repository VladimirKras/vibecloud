import { aiCapabilities, normalizeApplication } from "./application-model.ts";
import { readFile } from "node:fs/promises";
import { dirname } from "node:path";
import { z } from "zod";
import { functionGroups } from "./function-groups.ts";
...
).refine((value) => value !== "/_vibecloud" && !value.startsWith("/_vibecloud/"), "the /_vibecloud/ namespace is reserved for runtime media and release assets");
...
const assetOptions = { fallback: z.string().min(1).optional(), cloud_name: bucketName.optional() };
const assetsSchema = z.union([
  z.strictObject({ template: z.literal("vite"), build: buildSchema.optional(), ...assetOptions })
    .transform((asset) => ({ template: asset.template, ...(asset.fallback ? { fallback: asset.fallback } : {}), ...(asset.cloud_name ? { cloud_name: asset.cloud_name } : {}) })),
  z.strictObject({ template: z.literal("custom").optional(), build: buildSchema, ...assetOptions }),
]);
...
const functionSchema = z.strictObject({
  ...
  kind: z.enum(["http", "websocket", "timer", "stream"]).optional(),
  features: z.strictObject({
    ai: z.array(z.enum(aiCapabilities)).optional(),
    auth: z.enum(["provider", "session"]).optional(),
    public_media: z.boolean().optional(),
  }).optional(),
  database: resourceKey.optional(),
  bucket: resourceKey.optional(),
  runtime: z.string().regex(/^[a-z][a-z0-9]*$/, "must be a Yandex runtime identifier").optional(),
...
export const configSchema = z.strictObject(declarationShape).transform(normalizeApplication).superRefine((config, context) => {
  try {
    functionGroups(config);
  } catch (error) {
    context.addIssue({ code: "custom", path: ["functions"], message: (error as Error).message });
  }
  ...
  for (const [functionName, definition] of Object.entries(config.functions ?? {})) {
    if (definition.features?.public_media === true && (!definition.bucket || config.buckets?.[definition.bucket]?.public !== true)) {
      context.addIssue({ code: "custom", path: ["functions", functionName, "bucket"], message: "ai-image requires a declared public bucket; add a bucket with --public and select it with --bucket" });
    } else if (definition.bucket && !config.buckets?.[definition.bucket]) {
      context.addIssue({ code: "custom", path: ["functions", functionName, "bucket"], message: "references an unknown bucket" });
    }
    if (httpRoutedFunctions.has(functionName) && websocketRoutedFunctions.has(functionName)) {
      context.addIssue({ code: "custom", path: ["functions", functionName], message: "declare separate logical handlers for HTTP and WebSocket invocation types" });
    }
    ...
    if (definition.features?.auth === "session") {
      ...
      const hasMatchingAuth = authenticatedDatabase !== undefined && Object.values(config.functions ?? {}).some(
        (candidate) => candidate.features?.auth === "provider" && candidate.database === authenticatedDatabase,
      );
      if (!hasMatchingAuth) {
        context.addIssue({
          code: "custom",
    ...
    if (definition.features?.auth === "provider") {
      if (family !== "nodejs") {
    ...
    } else if (definition.database && !config.databases?.[definition.database]) {
      context.addIssue({ code: "custom", path: ["functions", functionName, "database"], message: "references an unknown database" });
    }
    if (definition.cron && definition.kind !== "timer") {
      context.addIssue({ code: "custom", path: ["functions", functionName, "kind"], message: "cron requires timer invocation" });
    }
    if (definition.kind === "timer" && !definition.cron) {
      context.addIssue({ code: "custom", path: ["functions", functionName, "cron"], message: "is required for timer invocation" });
    }
    if (definition.triggers?.length && definition.kind !== "stream") {
      context.addIssue({ code: "custom", path: ["functions", functionName, "kind"], message: "stream triggers require stream invocation" });
    }
    if ((httpRoutedFunctions.has(functionName) && definition.kind !== "http")
      || (websocketRoutedFunctions.has(functionName) && definition.kind !== "websocket")) {
      context.addIssue({ code: "custom", path: ["functions", functionName, "kind"], message: "invocation kind does not match its gateway routes" });
    }
