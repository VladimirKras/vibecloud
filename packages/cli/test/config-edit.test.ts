...
import { emptyProject, freshProject, prepareStateOnlyTerraform } from "./helpers.ts";
...
test("rename updates references, source paths, and Terraform moved blocks", async () => {
  ...
  assert.match(moves, /from = yandex_function\.functions\["stream-worker"\]/);
...
test("function renames preserve trigger identities across multiple consumers", async () => {
  ...
  await renameResource(configPath, "database", "primary", "data");
  assert.deepEqual(await triggerMoves(), [
    expected[0], ["consumer/primary.events", "consumer/data.events"],
    expected[1], ["consumer/primary.audit", "consumer/data.audit"],
    ["observer/primary.events", "observer/data.events"],
  ]);
  await renameStream(configPath, "data.events", "data.changes");
  assert.deepEqual(await triggerMoves(), [
    expected[0], ["consumer/primary.events", "consumer/data.events"], ["consumer/data.events", "consumer/data.changes"],
    expected[1], ["consumer/primary.audit", "consumer/data.audit"],
    ["observer/primary.events", "observer/data.events"], ["observer/data.events", "observer/data.changes"],
  ]);
});
