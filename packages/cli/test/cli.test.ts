...
const testGlobals = globalThis as typeof globalThis & {
  ...
  __vibecloudTestArt?: {
    images: { generate(request: { prompt: string, model?: string, size?: string }): Promise<unknown> }
  }
};
...
test("workspace packages share the CLI release and runtime contract", async () => {
  ...
  for (const name of [
    ...
    assert.equal(dependency.engines.node, ">=22.0.0");
  }
});
...
test("bundled CLI exposes the documented command surfaces", () => {
  ...
  const deletion = runCli(["delete", "--help"]);
  assert.equal(deletion.status, 0, deletion.stderr);
  assert.match(deletion.stdout, /--delete-after <duration>/);
  assert.match(deletion.stdout, /--status/);
  assert.match(deletion.stdout, /0s skips\s+the grace period/);
...
test("add auth creates a deployable Better Auth service, schema, and client", async () => {
  ...
  assert.deepEqual(loaded.config.functions?.auth, {
    kind: "http", runtime: "nodejs22", memory_mb: 128, timeout_seconds: 10, features: { auth: "provider" },
  ...
  assert.match(handler, /createYdbAuth\(/);
...
test("function --route creates an authenticated AI Studio Responses agent", async () => {
  ...
  assert.deepEqual(loaded.config.functions?.agent, {
    kind: "http", runtime: "nodejs22", memory_mb: 128, features: { ai: ["responses"], auth: "session" },
    template: "ai-agent",
    handler: "index.handler",
    database: "primary",
    timeout_seconds: 30,
...
export const requireAIStudioOutputText = (response) => {
  ...
  return response.output_text;
    ...
    "@vibecloud/db/better-auth": "export const createYdbAuth = () => ({ api: { getSession: async () => globalThis.__vibecloudTestSession } });",
    "@vibecloud/telemetry": `
...
export const withSpan = async (_name, _attributes, work) => work();`,
  ...
  const aliases = new Map([["@vibecloud/ai/http", join(cliRoot, "..", "ai", "src", "http.ts")], ["@vibecloud/storage", join(cliRoot, "..", "storage", "src", "index.ts")]]);
...
test("function --route creates a synchronous Alice AI ART endpoint", async () => {
  ...
  assert.deepEqual(loaded.config.functions?.illustrator, {
    kind: "http", runtime: "nodejs22", memory_mb: 256, timeout_seconds: 120, features: { ai: ["image_generation"], public_media: true },
    template: "ai-image",
    bucket: "images",
    handler: "index.handler",
  });
  ...
  }).functions?.illustrator.runtime, "python312");

  const source = await readFile(join(directory, "src", "functions", "illustrator", "index.ts"), "utf8");
  assert.match(source, /ai\.images\.generate/);
  assert.match(source, /requireAIStudioImage/);
  assert.doesNotMatch(source, /operationId|yandexArt/);
  const packageJson = await readJson(join(directory, "package.json"));
  assert.equal(packageJson.dependencies["@vibecloud/ai"], (await cliPackage()).version);
  assert.equal(packageJson.dependencies["@vibecloud/storage"], (await cliPackage()).version);
  assert.deepEqual(loaded.config.buckets?.images, { public: true });
  assert.equal(packageJson.dependencies["better-auth"], undefined);
  assert.equal(packageJson.dependencies["@vibecloud/db"], undefined);

  const terraform = await readFile(join(directory, "infra", "main.tf"), "utf8");
  assert.match(terraform, /image_generation_enabled\s*=\s*var\.deployment_plan\.ai\.image_generation/);
  assert.match(terraform, /image_generation_enabled\) \? \["ai\.models\.user"\]/);
...
export const AI_IMAGE_SIZES = ["auto", "1x1", "1024x1024", "1024x1536", "1536x1024"];
export const requireAIStudioImage = (response) => ({ image: new Uint8Array(3_147_780).fill(255), contentType: "image/png" });`,
    "@vibecloud/telemetry": `
...
export const withSpan = async (_name, _attributes, work) => work();`,
  ...
  const imageEnvironment = {
    IMAGES_BUCKET: "vc-images", VIBECLOUD_STORAGE_DIRECTORY: join(directory, ".vibecloud/media"),
    VIBECLOUD_STORAGE_URL: "/_vibecloud/media", VIBECLOUD_DEV_CONTAINER: "1",
  };
  const previousEnvironment = Object.fromEntries(Object.keys(imageEnvironment).map((key) => [key, process.env[key]]));
  Object.assign(process.env, imageEnvironment);
  const imageRequests: { prompt: string, model?: string, size?: string }[] = [];
  testGlobals.__vibecloudTestArt = {
    images: { generate: async (request) => {
      imageRequests.push(request);
      return { created: 1, data: [{ b64_json: "test-image" }] };
    } },
  };
  try {
    const generated = await import(`${pathToFileURL(bundledHandler).href}?test=${Date.now()}`);
    ...
    const completed = await invoke("POST", JSON.stringify({ prompt: "Paper city", size: "1024x1024" }));
    assert.equal(completed.statusCode, 200);
    const image = JSON.parse(completed.body);
    assert.equal(image.sizeBytes, 3_147_780);
    assert.match(image.url, /^\/_vibecloud\/media\/vc-images\/images\//);
    assert.ok(Buffer.byteLength(JSON.stringify(completed)) < 1_024);
    assert.ok(!completed.isBase64Encoded);
    assert.match(completed.headers["content-type"], /application\/json/);
    assert.equal(completed.headers["cache-control"], "no-store");
    assert.equal((await invoke("GET", "", "old-operation")).statusCode, 405);
    const encoded = await invoke("POST", JSON.stringify({ prompt: "Бумажный город 🌆" }), undefined, true);
    assert.equal(encoded.statusCode, 200);
    assert.equal(imageRequests[1].prompt, "Бумажный город 🌆");
    assert.equal(imageRequests[0].model, "aliceai-image-art-3.0");
    for (const body of [null, [], { prompt: "x", seed: 4 }, { prompt: "x", aspectRatio: { widthRatio: 1, heightRatio: 1 } }, { prompt: "x", size: "bad" }]) {
      assert.equal((await invoke("POST", JSON.stringify(body))).statusCode, 400);
    }
    assert.equal((await invoke("POST", JSON.stringify({ prompt: "x".repeat(501) }))).statusCode, 413);
...
test("image scaffolding cannot silently open an existing private bucket", async () => {
  const { directory, configPath } = await emptyProject("private-images-app");
  assert.equal(runCli(["add", "bucket", "images"], directory).status, 0);
  const before = await readFile(configPath, "utf8");
  const rejected = runCli(["add", "function", "image", "--template", "ai-image", "--route", "/api/images"], directory);
  assert.notEqual(rejected.status, 0);
  assert.match(rejected.stderr, /public bucket/);
  assert.equal(await readFile(configPath, "utf8"), before);
  const publicBucket = runCli(["add", "bucket", "gallery", "--public"], directory);
  assert.equal(publicBucket.status, 0, publicBucket.stderr);
  const created = runCli(["add", "function", "image", "--template", "ai-image", "--bucket", "gallery", "--route", "/api/images"], directory);
  assert.equal(created.status, 0, created.stderr);
  assert.equal((await loadConfig(configPath)).config.functions?.image.bucket, "gallery");
  assert.equal(runCli(["rename", "bucket", "gallery", "pictures"], directory).status, 0);
  assert.equal((await loadConfig(configPath)).config.functions?.image.bucket, "pictures");
  assert.match(await readFile(join(directory, "src/functions/image/index.ts"), "utf8"), /PICTURES_BUCKET/);
});
...
test("function --route creates a serverless multimodal AI turn", async () => {
  ...
  assert.deepEqual(loaded.config.functions?.assistant, {
    kind: "http", runtime: "nodejs22", features: { ai: ["responses", "speechkit_stt", "speechkit_tts"], auth: "session" },
    template: "ai-turn",
    handler: "index.handler",
    database: "primary",
    memory_mb: 256,
  ...
  await writeFile(typecheckConfig, `${JSON.stringify({
    compilerOptions: {
      ...
      paths: {
        ...
        "@vibecloud/ai/http": [join(cliRoot, "..", "ai", "src", "http.ts")],
...
`);
  ...
  await writeFile(mockDbAuth, "export const createYdbAuth = () => ({ api: { getSession: async () => globalThis.__vibecloudTestSession } });\n");
...
test("function --route atomically creates a websocket function", async () => {
  ...
  assert.deepEqual(loaded.config.functions?.game, {
    kind: "websocket", runtime: "nodejs22", memory_mb: 128, timeout_seconds: 10, features: {},
    template: "websocket",
    handler: "index.handler",
  });
  assert.deepEqual(loaded.config.gateway.routes, [{
...
test("function --cron atomically scaffolds and schedules a timer function", async () => {
  ...
  assert.deepEqual(loaded.config.functions?.cleanup, {
    kind: "timer", runtime: "nodejs22", memory_mb: 128, timeout_seconds: 10, features: {},
