...
test("local YDB uses the project-isolated OrbStack hostname", () => {
  assert.deepEqual(localProjectLocation("billing-app"), {
...
test("development requires the OrbStack engine, with no published-port fallback", async () => {
  await assert.rejects(requireOrbStack("/workspace", async (_command, args) => args[0] === "info" ? "Docker Desktop" : "2.30.0"), /requires OrbStack/);
  await requireOrbStack("/workspace", async (_command, args) => args[0] === "info" ? "OrbStack" : "2.30.0");
});
...
test("local gateway invokes a built Node function with the cloud event shape", async () => {
  ...
  await writeFile(join(output, "router.js"), `exports.handler = async (event, context) => ({
    statusCode: 201,
