...
test("Terraform template declares the current deployment resources", async () => {
  ...
  assert.match(terraform, /for_each\s+= local\.function_groups/);
  assert.match(terraform, /local\.artifacts\}\/functions\/\$\{each\.key\}/);
...
test("Terraform asset integrations only emit configured fallback objects", async () => {
  ...
  assert.equal((terraform.match(/"error_object" => "\$\{local\.asset_prefixes\[route\.assets\]\}\$\{fallback\}" if/g) ?? []).length, 4);
