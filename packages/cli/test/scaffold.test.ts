...
test("doctor repairs a missing ownership label while preserving existing labels", async () => {
  const { directory } = await emptyProject("repair-label-app");
  const metadata = await readJson(join(directory, ".vibecloud", "project.json"));
  let labels: Record<string, string> = { owner: "team" };
  const result = await doctorProject(directory, {
    repair: true,
    environment: { YC_TOKEN: "token", YC_CLOUD_ID: "cloud" },
    readCommand: async (command) => command === "yc"
      ? JSON.stringify({ id: metadata.yc_folder_id, status: "ACTIVE", labels })
      : "available",
    runCommand: async (command, arguments_) => {
      assert.equal(command, "yc");
      assert.deepEqual(arguments_.slice(0, 3), ["resource-manager", "folder", "update"]);
      labels = Object.fromEntries(arguments_[arguments_.indexOf("--labels") + 1].split(",").map((label) => label.split("=")));
    },
  });
  assert.equal(result.healthy, true);
  assert.equal(result.repaired, true);
  assert.deepEqual(labels, { owner: "team", vibecloud_project_id: metadata.project_id });
});

test("doctor cannot replace another project's ownership label during repair", async () => {
  const { directory } = await emptyProject("conflicting-label-app");
  const metadata = await readJson(join(directory, ".vibecloud", "project.json"));
  const writes: string[] = [];
  const result = await doctorProject(directory, {
    repair: true,
    environment: { YC_TOKEN: "token", YC_CLOUD_ID: "cloud" },
    readCommand: async (command) => command === "yc"
      ? JSON.stringify({ id: metadata.yc_folder_id, status: "ACTIVE", labels: { vibecloud_project_id: "another-project" } })
      : "available",
  ...
  assert.match(result.checks.find((check) => check.name === "repair")!.message, /different Vibecloud project/);
});
...
test("resource scaffolds materialize the current source and package contract", async () => {
  ...
  assert.doesNotMatch(localCompose, /image: ydbplatform/);
  ...
  assert.doesNotMatch(localCompose, /"2136:2136"/);
  assert.doesNotMatch(localCompose, /\bports:/);
  assert.match(localCompose, /dev.orbstack.domains/);
  assert.match(
...
test("resource scaffolding updates pnpm package metadata", async () => {
  ...
  assert.deepEqual((await loadConfig(configPath)).config.assets?.admin, { template: "vite" });
});
...
test("build orchestration invokes the configured project build once", async () => {
  ...
  assert.equal(await buildProject(loaded, {
    runCommand: async (command, arguments_, environment, cwd) => { commands.push({ command, arguments_, environment, cwd }); },
  }), true);
  assert.deepEqual(commands, [{ command: "pnpm", arguments_: ["build"], environment: { ...process.env, VIBECLOUD_CONFIG_PATH: loaded.configPath }, cwd: loaded.rootDirectory }]);
});

test("generated production builds load the project Vite configuration", async () => {
  const source = await readFile(join(cliRoot, "src", "application-build.ts"), "utf8");
...
test("Python and Go functions scaffold and build into artifact directories", async () => {
  ...
  const build = spawnSync(process.execPath, ["build.ts"], { cwd: directory, encoding: "utf8" });
  assert.equal(build.status, 0, build.stdout + build.stderr);
  await stat(join(directory, "dist", "functions", "http-python312", "handlers", "h0", "index.py"));
  await stat(join(directory, "dist", "functions", "stream-ingester", "index.go"));
...
async function typecheckProject(directory: string) {
  ...
  for (const [dependency, owner] of [
    ...["@types/node", "typescript"].map((name) => [name, cliRoot]),
