...
application feature or library defines the required schema. Different identities with
identical SQL are rejected rather than silently skipped by the adapter's hash identity.

Failed and interrupted migrations stop normal execution. Inspect partial effects and
verify that the previous owner stopped before using
`pnpm vibecloud db up --retry-interrupted`. It replays from the first statement, keeps
locking and immutable checks, and retries running records only after the adapter's
one-hour stale threshold. The SDK equivalent, including for an isolated local endpoint,
is `migrateYdbFolder(endpoint, folder, { recovery: "retry" })`. A replay can repeat
already-executed statements; make recovery deliberate and never edit applied history.
...
`@ydbjs/drizzle-adapter@0.1.1` types `select()` results as `unknown[]`, including
explicit selections. Narrow a returned row and validate the fields the handler
reads before property access. This is a type-inference limitation, not a failed
query. Keep using the invocation's Drizzle database; do not add another driver or
ORM just to suppress the type error. Recheck the installed adapter's declarations
when upgrading, rather than treating this version-specific limitation as permanent.
