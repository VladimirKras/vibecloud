...
`VIBECLOUD_FUNCTION_OUTPUT` and must write artifacts to the supplied output
directory. Do not hard-code `dist/`; deployment uses an isolated build directory.
...
`@vibecloud/ai`, and infer their required IAM roles. These are template
requirements: custom `api` handlers can use the SDK without Better Auth or YDB.
See the [custom AI handler recipe](ai.md#custom-ai-handlers), and
[ai.md](ai.md) for authentication, additional AI capabilities, and the
Realtime relay boundary.

`ai-image` is a public Node.js Alice AI ART endpoint. POST accepts a prompt of
at most 500 Unicode characters and an optional supported size, then returns
JSON `{ key, url, contentType, sizeBytes }`. Render the returned URL directly.
It creates a public `images` bucket by default, or uses `--bucket <name>` for an
existing public bucket. The function uploads raw bytes using its IAM token; no
signing is needed. It uses the synchronous Images API,
with a default timeout of 120 seconds and 256 MB of memory. It adds
`@vibecloud/ai` and `@vibecloud/storage` and infers `ai.models.user` and storage
permissions without requiring Better Auth or YDB.
There is no operation polling or automatic generation retry.
...
Add Better Auth when the application needs accounts. It is an optional feature,
not a requirement for ordinary HTTP, static assets, or custom AI handlers.
...
shared configured cluster (default `default`), and the compiled deployment group
as its service, for example `http-nodejs22`. Query it with a selector such as
`{project="folder__<project-folder-id>", cluster="default", service="http-nodejs22"}`.
Node invocation attributes identify the logical handler as `vibecloud.function.name`.
...
function invocation ID on the span. The build-time invocation wrapper records
HTTP and messaging attributes. Application logs emitted with
`structuredLog` carry request, trace, and span correlation.

Enabling telemetry does not rewrite existing handlers. The generated builder
instruments Node.js exports from the current observability configuration on
every build. Use `pnpm vibecloud orphans` to inspect retained source and
dependencies after resource removal.

Use `pnpm vibecloud list [kind]` with `asset`, `bucket`, `database`, `stream`,
`function`, `route`, `trigger`, `cron`, or `secret`.

Buckets accept `public: true` (`add bucket <name> --public`) to allow anonymous
object reads, with listing disabled and writes authenticated. Ordinary buckets
are private by default. `ai-image` functions declare their public `bucket` binding;
renaming a bucket updates the binding and generated environment reference.
The internal URL namespace `/_vibecloud/` is reserved for media and release assets.

## Explicit function policy

Generated/updated declarations materialize `kind` (`http`, `websocket`, `timer`,
`stream`), `runtime`, memory, timeout, and `features`. Features can grant `ai`
capabilities (`responses`, `realtime`, `speechkit_stt`, `speechkit_tts`,
`image_generation`), declare `auth: "provider" | "session"`, or require
`public_media: true`. A template is source-generation provenance; its defaults are
expanded once and persisted. Removing its label preserves explicit policy. Changing
features must accompany corresponding handler changes, such as adding authentication.
