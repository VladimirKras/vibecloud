...
0. **Finish dependency installation first.** If a scaffold or resource command
   printed `updated: package.json`, run the project's install command before
   inspecting docs or imports. A package declared in `package.json` is not an
   installed package until that install succeeds.
...
2. **Fetch the docs for the package you are about to use, in this order,
   ...
   (a) **LOCAL** — enumerate the installed package with
      `rg --files -L node_modules/@gravity-ui/<pkg>`; then read its real
      `dist/docs/INDEX.md` or `build/docs/INDEX.md` and the matching component
      document (often under `docs/components/`). Do not construct a component
      path before enumerating it.
...
- **Never guess icon names** — they are hallucinated more often than not. To
  ...
  (reverse image search). With only a description, use the online catalog.
  Before importing any candidate, confirm its exact export in the installed
  `node_modules/@gravity-ui/icons/index.d.ts`; for example,
  `rg 'default as ArrowDownToLine' node_modules/@gravity-ui/icons/index.d.ts`.
  Then pass it as an
