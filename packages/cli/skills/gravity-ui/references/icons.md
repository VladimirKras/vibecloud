...
Use the catalog for visual/text discovery:
...
Before importing a catalog/image candidate, confirm the exact name in the
installed package:
...
No match means that version cannot import it. Search the installed declaration
for semantic fragments such as `Down` and `Line`, then choose only a confirmed
export. Installed exports are the final authority. Do not guess from memory.
