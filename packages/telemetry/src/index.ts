import { safeLogValue, telemetryEffect } from "./diagnostics.js";
...
const flushTimeoutMillis = invocationCleanupMillis - 500;

const provider = optionalTelemetry(createProvider);
const meterProvider = optionalTelemetry(createMeterProvider);
const loggerProvider = optionalTelemetry(createLoggerProvider);
...
function optionalTelemetry<T>(factory: () => T): T | undefined {
...
export async function traceInvocation<T>(
  ...
  if (invocationStorage.getStore()?.requestId === invocation.requestId) return work();
  ...
  return tracer.startActiveSpan(name, {
    ...
    try {
      ...
      telemetryEffect(() => recordError(error));
      throw error;
    } finally {
      telemetryEffect(() => span.end());
      await flushTelemetry(remainingInvocationMillis(invocation, 100, flushTimeoutMillis + 100)).catch(() => undefined);
    }
  }));
}
...
export function traceOperation<T>(
  ...
  return operationTracer.startActiveSpan(operationName, async (span) => {
    telemetryEffect(() => span.setAttributes(attributes));
    try {
      return await operation({
        setAttributes: (nextAttributes) => telemetryEffect(() => span.setAttributes(nextAttributes)),
      });
    } catch (error) {
...
export function setSpanAttributes(attributes: Attributes): void {
  telemetryEffect(() => trace.getSpan(context.active())?.setAttributes(attributes));
}
...
export function createStructuredLogger(streamName: string) {
  ...
  ): void => telemetryEffect(() => emitStructuredLog(streamName, streamLogger, level, message, attributes));
}
...
export function structuredLog(
  ...
  telemetryEffect(() => emitStructuredLog("application", logger, level, message, attributes));
}
...
function emitStructuredLog(
  ...
  const fields: LogAttributes = {
    ...
    ...safeLogValue(attributes) as Record<string, LogAttributes[string]>,
  ...
  if (otelLogger) {
    telemetryEffect(() => otelLogger.emit({
      eventName: typeof fields["event.name"] === "string"
        ? fields["event.name"]
        : undefined,
      body: message,
...
async function flushTelemetry(timeoutMillis: number): Promise<void> {
  ...
  const flushes = [
    ...
    loggerProvider?.forceFlush({ timeoutMillis }).catch((error: unknown) => {
...
function writeConsoleLog(
  ...
  const entry = JSON.stringify({ ...safeLogValue(attributes) as Record<string, unknown>, message: message.slice(0, 8_192), level, stream_name: streamName });
  telemetryEffect(() => {
...
/** Instrument the exported handler at build time without rewriting authored source. */
export function instrumentFunction<Event, Context extends InvocationContext, Result>(
  handler: (event: Event, context: Context) => Result | Promise<Result>,
): (event: Event, context: Context) => Promise<Result> {
  return async (event, invocation) => {
    const value = event as { httpMethod?: string, resource?: string, path?: string, headers?: Record<string, string>, messages?: unknown[], requestContext?: { eventType?: string, connectionId?: string, requestId?: string } };
    const route = value.resource ?? value.path ?? "invocation";
    return traceInvocation(`${value.httpMethod ?? "function"} ${route}`, invocation, {
      carrier: value.headers,
      kind: value.httpMethod ? SpanKind.SERVER : SpanKind.CONSUMER,
      attributes: {
        ...(invocation.logicalFunctionName ? { "vibecloud.function.name": invocation.logicalFunctionName } : {}),
        "yandex.apigateway.request_id": value.requestContext?.requestId ?? invocation.requestId,
        ...(value.httpMethod ? { "http.request.method": value.httpMethod, "http.route": route } : {}),
        ...(value.messages ? { "messaging.batch.message_count": value.messages.length } : {}),
        ...(value.requestContext?.connectionId ? { "vibecloud.websocket.connection_id": value.requestContext.connectionId } : {}),
      },
    }, async () => {
      const result = await handler(event, invocation);
      const status = (result as { statusCode?: unknown } | undefined)?.statusCode;
      if (typeof status === "number") telemetryEffect(() => setSpanAttributes({ "http.response.status_code": status }));
      return result;
    });
  };
}
