...
test("generates Alice AI ART images synchronously with scoped credentials", async () => {
  const calls: CapturedRequest[] = [];
  const imageBytes = Buffer.from("iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/x8AAwMCAO+jZxkAAAAASUVORK5CYII=", "base64");
  const client = createAIStudioClient({ functionFolderId: "folder-1", token: { access_token: "iam-token" } }, {
    environment: {},
    fetch: async (url, init = {}) => {
      calls.push({ url: String(url), init });
      return Response.json({ created: 1, data: [{ b64_json: imageBytes.toString("base64") }] });
    },
  });
  const response = await client.images.generate({ prompt: " A paper city ", size: "1536x1024" });
  const image = requireAIStudioImage(response);
  assert.deepEqual(Buffer.from(image.image), imageBytes);
  assert.equal(image.contentType, "image/png");
  assert.equal(calls.length, 1);
  assert.equal(calls[0].url, "https://ai.api.cloud.yandex.net/v1/images/generations");
  assert.equal(new Headers(calls[0].init.headers).get("authorization"), "Bearer iam-token");
  assert.equal(new Headers(calls[0].init.headers).get("openai-project"), "folder-1");
  assert.equal(new Headers(calls[0].init.headers).get("x-data-logging-enabled"), "false");
  assert.deepEqual(jsonBody(calls[0].init.body), {
    model: "art://folder-1/aliceai-image-art-3.0", prompt: "A paper city", size: "1536x1024",
  });
  await client.images.generate({ prompt: "City", model: "art://other-folder/custom-image", size: "auto" });
  assert.equal(jsonBody(calls[1].init.body).model, "art://other-folder/custom-image");
});

test("image validation rejects unsupported requests before network access", async () => {
  let calls = 0;
  const client = createAIStudioClient({}, { folderId: "folder", iamToken: "token", environment: {}, fetch: async () => {
    calls++;
    return Response.json({ created: 1, data: [] });
  } });
  await assert.rejects(() => client.images.generate({ prompt: " " }), /non-empty/);
  await assert.rejects(() => client.images.generate({ prompt: "x".repeat(501) }), /500 characters/);
  // @ts-expect-error Unsupported provider option, also rejected at runtime.
  await assert.rejects(() => client.images.generate({ prompt: "City", seed: 42 }), /supports only/);
  // @ts-expect-error Old API aspect ratios are not Images API sizes.
  await assert.rejects(() => client.images.generate({ prompt: "City", size: "16x9" }), /Unsupported image size/);
  assert.equal(calls, 0);
  await client.images.generate({ prompt: "🌆".repeat(500), size: "1x1" });
  assert.equal(calls, 1);
});

test("image results require base64 raster data and detect the actual format", () => {
  for (const b64_json of [undefined, "", "not base64", Buffer.from("<svg/>").toString("base64")]) {
    assert.throws(() => requireAIStudioImage({ created: 1, data: [{ b64_json }] }), AIStudioImageError);
  }
  assert.throws(() => requireAIStudioImage({ created: 1, data: [{ url: "https://example.com/image" }] }), /no generated image/);
  for (const [bytes, contentType] of [
    [Buffer.from([0xff, 0xd8, 0xff, 0xe0]), "image/jpeg"],
    [Buffer.from("RIFF0000WEBPVP8 "), "image/webp"],
  ] as const) {
    assert.equal(requireAIStudioImage({ created: 1, data: [{ b64_json: bytes.toString("base64") }] }).contentType, contentType);
  }
});
...
test("discarded Responses deletion bodies release their transport", async () => {
  let cancelled = false;
  const client = createAIStudioClient({}, { folderId: "fixture", iamToken: "fixture", environment: {}, fetch: async () => new Response(new ReadableStream({ cancel() {
    cancelled = true;
  } })) });
  await client.responses.delete("fixture");
