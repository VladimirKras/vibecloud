...
if (stable && provenance && !provenance.tagCommit) {
  run("git", [
    ...
    "-m",
    `Verified complete Verdaccio release ${publishVersion}.`,
  ], root);
...
async function publishReleaseArtifacts(artifacts: ReleaseArtifact[]) {
  ...
  const states = await Promise.all(artifacts.map(async (artifact) => {
    ...
    if (status.kind === "mismatched") throw new Error(`${artifact.name}@${publishVersion} already has different contents`);
    if (stable && status.kind === "matching" && metadata["dist-tags"]?.[stagingTag] !== publishVersion) assertMatchingLatest(metadata, artifact.name, publishVersion);
    return { artifact, status };
  }));
  for (const { artifact, status } of states) {
    if (status.kind === "matching") setDistTag(artifact, stagingTag);
    else publishArtifact(artifact, stagingTag);
  }
  ...
  run(process.execPath, [join(root, "scripts/registry-project-acceptance.ts"), publishVersion], root);
  if (process.env.VIBECLOUD_LIVE_YC === "1") run(process.execPath, [join(root, "scripts/live-yc-acceptance.ts"), publishVersion], root);
  ...
  console.log(`verified complete ${artifacts.length}-package release ${publishVersion}`);
}
