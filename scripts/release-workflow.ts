...
export function githubSnapshotPushArguments(snapshot: string, branch: string, observedCommit: string): string[] {
  if (observedCommit && !/^[0-9a-f]{40,64}$/u.test(observedCommit)) throw new Error("remote snapshot must be a full Git object ID");
  return ["push", `--force-with-lease=refs/heads/${branch}:${observedCommit}`, "origin", githubSnapshotRefspec(snapshot, branch)];
}
