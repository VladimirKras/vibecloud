...
for (const packageName of releasePackageDirectories) {
  ...
  const runtime = !["cli", "codex-skill-init"].includes(packageName);
  assert(value.engines?.node === (runtime ? ">=22.0.0" : ">=26.0.0"), `${value.name}: incorrect runtime/tooling engine contract`);
...
function packageSpecificFiles(packageName: string) {
  if (packageName === "@vibecloud/cli") return ["ARCHITECTURE.md"];
