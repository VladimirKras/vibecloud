...
// Unconfirmed cloud cleanup receipts must never expire with diagnostic artifacts.
const LIVE_ACCEPTANCE_TTL_MS = Number.POSITIVE_INFINITY;
...
if (!version || !/^\d+\.\d+\.\d+(?:-[A-Za-z0-9.-]+)?$/.test(version)) throw new Error("Live acceptance requires the exact published candidate version as an argument");
...
try {
  mkdirSync(driver);
  writeFileSync(join(driver, ".npmrc"), `registry=${registry}\n@vibecloud:registry=${registry}\n`);
  ...
  run("pnpm", ["install"], driver, environment);
  ...
  const deployment = JSON.parse(readFileSync(join(project, "infra/.packages/current-deployment.json"), "utf8")) as { inputs: string };
  run("terraform", ["-chdir=infra", "plan", `-var-file=${deployment.inputs}`, "-input=false", "-lock=false", "-detailed-exitcode", "-no-color"], project, terraformEnvironment);
  ...
  const metadataPath = join(project, ".vibecloud/project.json");
  if (!folderId && existsSync(metadataPath)) folderId = JSON.parse(readFileSync(metadataPath, "utf8")).yc_folder_id;
  if (folderId) {
    try {
      run(process.execPath, [cli, "delete", "--confirm", `delete:${name}`, "--delete-after", "0s"], project, environment);
      await retryUntil("live acceptance folder deletion", 180_000, async () => {
        run(process.execPath, [cli, "delete", "--status"], project, environment);
        const receipt = JSON.parse(readFileSync(metadataPath, "utf8")).deletion;
        assert.equal(receipt?.status, "deleted");
      });
      console.log(`confirmed deletion of live acceptance folder ${folderId}`);
    } catch (error) {
...
async function waitForFunctionLogs(functionKey: string, required: string[], timeoutMs: number) {
  assert.ok(folderId);
  const functionId = deployedFunctionId(functionKey);
  return retryUntil(`${functionKey} logs`, timeoutMs, async () => {
    const logs = capture("yc", [
      "logging", "read", "--group-name", "default", "--folder-id", folderId,
      "--resource-ids", functionId, "--since", startedAt,
...
function printFunctionDiagnostics(functionKeys: string[]) {
  ...
  for (const functionKey of functionKeys) {
    try {
      const logs = capture("yc", [
        "serverless", "function", "logs", "--id", deployedFunctionId(functionKey),
...
function deployedFunctionId(logicalName: string): string {
  const current = JSON.parse(readFileSync(join(project, "infra/.packages/current-deployment.json"), "utf8"));
  const input = JSON.parse(readFileSync(current.inputs, "utf8"));
  const group = input.deployment_plan.function_group_keys[logicalName];
  if (!group) throw new Error(`No compiled group for ${logicalName}`);
  const state = JSON.parse(capture("terraform", ["-chdir=infra", "show", "-json"], project, environment));
  const resource = state.values.root_module.resources.find((entry: { type: string, name: string, index?: string }) => entry.type === "yandex_function" && entry.name === "functions" && entry.index === group);
  if (!resource?.values?.id) throw new Error(`No deployed function for ${logicalName} (${group})`);
