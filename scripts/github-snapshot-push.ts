...
// Capture before constructing the snapshot. A later publisher must not be overwritten.
const remoteHead = readCommand("git", ["ls-remote", "--refs", repositoryUrl, `refs/heads/${branch}`]).split(/\s+/)[0] ?? "";
...
try {
  ...
  run("git", githubSnapshotPushArguments(snapshot, branch, remoteHead), {
    cwd: temporaryRepository,
    env: temporaryEnvironment,
  });
} finally {
